...
*}
<div id="formPayonePanel" class="panel{if $isPrestaShop1770rHigher} card{/if}">
  <div class="{if $isPrestaShop1770rHigher}card-header{else}panel-heading{/if}">
    {if $isPrestaShop1770rHigher}<h3 class="card-header-title">{/if}
      <i class="icon-money"></i>
      {$oFcPayoneTranslator->translate('FC_PAYONE_BACKEND_ORDER_PANEL_TITLE')|escape:'html':'UTF-8'}
      {if $isPrestaShop1770rHigher}</h3>{/if}
  </div>
  <div class="{if $isPrestaShop1770rHigher}card-body {else}panel-body{/if}">
    ...
    <div class="{if !$isPrestaShop1770rHigher}row{/if}">
